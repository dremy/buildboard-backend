#! /bin/bash
# Automated script to build TFA core
#
...
# As a sanity check, also make sure the prod server is one of the two web heads.
# We always want to make sure we build in release mode when we're on production.
for i in ${restrict[@]}; do
  if [ "$i" == "$HOSTNAME" ] && [ $1 != "-r" ]; then
    echo
    echo '--> It seems like you are trying to rebuild the site in dev mode, which is not allowed.'
    echo '--> Please add the -r parameter and try again.  See below for example.'
    echo
    echo '--> ./build.sh -r ../drupal'
    echo
    exit;
  fi
done
...
ln -s "$CURDIR/profiles/buildboard" $TARGET_PATH/profiles/buildboard
echo "--> Complete!"
...
# echo
# echo '--> Compiling SASS...'
# cd "$ORIGINAL/profiles/tfa/themes/tfa"
# compass compile;
# echo "--> Complete!"
...
# Note: You want to run updatedb.sh on the database servers upon rebuilding the site root.
